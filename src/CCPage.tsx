...
function MessageRow({ message, expanded, onToggleThinking }: {
  ...
  if (message.role === 'activity') {
    ...
    return (
      <div className="cc-msg activity-row">
        {acts.map((a: any, i: number) => {
          const tool = a.tool || a.name || ''
          const detail = a.detail || a.content || ''
          return (
            <div key={i} className="cc-activity-line">
              苏煦在用 <span className="cc-act-tool">{tool || '工具'}</span>
              {detail && <span className="cc-act-detail">：{String(detail).slice(0, 200)}</span>}
            </div>
          )
        })}
        {acts.length === 0 && <div className="cc-activity-line">苏煦在用工具</div>}
      </div>
    )
  }
  const text =
  ...
  return (
    <div className={`cc-msg ${message.role}${message.pending ? ' pending' : ''}`}>
      ...
      <div className="cc-text-col">
        ...
        {text && <div className="cc-text">{text}</div>}
      </div>
    </div>
  )
}
  ...
  return (
    <div className="cc-modal-backdrop" onClick={onClose}>
      <div className="cc-modal cc-panel glass" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="cc-modal-actions">
          <button
            className="cc-panel-action"
            onClick={() => onAction('session_forge')}
            title="压缩当前 session 释放上下文"
          >compact</button>
          <button
            className="cc-panel-action"
            onClick={() => onAction('session_rotate')}
